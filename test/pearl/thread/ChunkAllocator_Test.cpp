...
TEST_P(ChunkAllocatorT, allocate_allocator_returnsNonNullptr)
{
...
TEST_P(ChunkAllocatorT, allocate_allocator_returnsUniquePointers)
{
    for (size_t i = 0; i < MAX_BLOCKS; ++i)
    {
        void* const ptr = mAllocator->allocate();
...
TEST_P(ChunkAllocatorT, allocate_afterDeallocate_returnsSamePointer)
{
    for (size_t i = 0; i < MAX_BLOCKS; ++i)
    {
        void* const ptr = mAllocator->allocate();
...
TEST_P(ChunkAllocatorT, deallocate_fifoOrder_works)
{
    for (size_t i = 0; i < MAX_BLOCKS; ++i)
    {
        void* const ptr = mAllocator->allocate();
...
TEST_P(ChunkAllocatorT, deallocate_randomOrder_works)
{
    for (size_t i = 0; i < MAX_BLOCKS; ++i)
    {
        void* const ptr = mAllocator->allocate();

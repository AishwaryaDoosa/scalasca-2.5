...
bool includes_omp_instrumentation(const string& target)
{
...
bool includes_pthread_instrumentation(const string& target)
{
...
string
numberWithLeadingZeroes(int value, int max)
{
...
int main (int argc, char* argv[])
{
    ...
    while ((ch = getopt(argc, argv, "+hvnqstae:f:l:m:R:OM:")) != EOF) {
        switch(ch) {
            case 'e':
                archive = optarg;
            ...
            case 'm':
                ...
                break;
            case 'M':
                multirun_cfg.load(optarg);
                if (verbose) cerr << "Got multi-run configuration file \"" << optarg << "\"" << endl;
                break;
            case 'n':
                preview=true;
                break;
            ...
            case 'R':
                ...
                if (verbose) cerr << "Number of measurement runs per config to be done: " << runs_per_config << endl;
                break;
    ...
    string blacklist = multirun_cfg.checkEnvBlacklist();
    if ( !blacklist.empty() )
    {
      cerr << "Score-P or Scalasca variables are set in the global environment for a\n"
           << "multi run experiment with config file! Please use the global variables\n"
           << "section of the config file for these variables!\n"
           << "Offending variables:" << blacklist << endl;
          exit(1);
    }
    ...
    if ((epk_archive_exists("/bgl/BlueLight") || epk_archive_exists("/bgsys")) &&
        ...
        if (!launch_options.cnmode) { // set default mode according to system
            if (launcher=="runjob") launch_options.cnmode=UTILS_CStr_dup("1p");
    ...
    if (!archive.empty()) { // override title with specified archive
    ...
    if (runs_per_config > 1 || multirun_cfg.getNumSets() > 1)
    {
      ...
      if (measure)
      {
        ...
        retval = mkdir(dirname.c_str(), S_IRWXU | S_IROTH | S_IROTH);
        if (retval != 0) {
...
for (int j=0; j < runs_per_config; j++)
{
    ...
    // the MPI tasks need to be specified in MPIEXPORT
    {
        ...
        if (!pipe)
            ...
            while (fgets(confvar, 1024, pipe)) {
                ...
                *val='\0'; val++;
                if (verbose>1) cerr << "Got config-var: " << confvar << envassign << val << endl;
                if (exp_vars.end() == find(exp_vars.begin(), exp_vars.end(),
                                           string(confvar)))
    ...
    if ((!measure || (measure && tracing)) && (analcmd != "none")) {
        if (analyzer.find("scout.ser") != string::npos) olaunch=true;
...
// and indeed appears to introduce problems!
            ...
            if (!verified) {
                ...
                status = WEXITSTATUS(retval);
                if (status) cerr << report << ": " << strerror(status) << endl;
                exit(4);
            } else {
                off_t kbytes = 0;

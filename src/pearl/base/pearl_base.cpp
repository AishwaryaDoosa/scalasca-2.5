...
namespace {
    ...
    // *INDENT-OFF*
    void determineTimeOffset(MutableGlobalDefs& defs, const LocalTrace& trace);
...
void pearl::PEARL_verify_calltree(GlobalDefs& defs, LocalTrace& trace)
{
  ...
  if ((threadId == 0) && event.is_valid() && event->isOfType(ENTER_PROGRAM))
  {
...
///
void
determineTimeOffset(MutableGlobalDefs& defs, const LocalTrace& trace)
{

...
bool ReadDefinitions(TaskDataShared& sharedData);
...
// ----- MPI-specific functions -----
    ...
    bool ValidateParallelizationMPI(const GlobalDefs& defs);
...
// ----- OpenMP-specific functions -----
    ...
    bool ValidateParallelizationOMP(int num_threads);
                 ...
                 char** argv)
{
    ...
    while (index < argc)
    {
        ...
        else
        {
                ...
                if ((arg == "--time-correct") || (arg == "--no-time-correct"))
                {
                    LogMsg(0,
